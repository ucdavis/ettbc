...
%macro nfu99(test= ,hcpcs=, margin=, one=1, datehcpcs= ,id= , year= ,fileopc= , fileopr= , filecarl= , filecarh= , cpt_right_end= , fromdt= , thrudt= ,hcpcsfield= ,posfield= ,claimid= , claimid_car= ,pos= );
...
data test_car_pre1;
set &filecarl (keep= &id &hcpcsfield /*&datehcpcs &thrudt*/ &claimid_CAR expnsdt1 expnsdt2  /*obs=5000000*/) ;
...
keep &id &test date_&test file_&test ; 
if &test=1;
run;

data test_car_pre2;
set &filecarh (keep= &id &posfield bexpdt1  /*obs=500000*/) ;
...
data test_car_pre;
set test_car_pre1 test_car_pre2;
