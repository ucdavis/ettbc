/*************************************************************************/
/* Code to compute bootstrap variance in the 75-84 age group */
/*************************************************************************/
...
%macro cann23(agein1= , agein2= , agein3= ,agein4= ,agein5=, agein6=, agein7=, agein8=, agein9=, agein10=, agegroup= ,from= ,to= ,event= ,debugging= );
...
set anndata.long12m7579 (&debugging) anndata.long12m8084 (&debugging) ;
run;
...
data longcovs&agegroup; set anndata.longcovs7579 (&debugging) anndata.longcovs8084 (&debugging); 

newbeneid=cats(of bene_id age); drop bene_id ; run;
...
data basecovs7579;
set anndata.longcovs7579 (where=(month2=0) keep= 
...
data basecovs8084;
set anndata.longcovs8084 (where=(month2=0) keep= 
bene_id age month2 year_base race_c region 
ervisit6m_base2 daysin6m_base2 
 baseline_combinedscore
cvd_prev_base crc_prev_base diabetes_prev_base pelvic_prev_base influenza_base bonemass_base 
...
/* #4 */
  proc sql;
  create table bootsample&i as select * from bootstudysingleids&i full join longcovs_all7084 (rename=(bene_id=bene_id2)) on bootstudysingleids&i..bene_id=longcovs_all7084.bene_id2;
  quit;
  /* watch out, delte this later */ data bootsample&i; set bootsample&i; if bene_id ne '' ; run;
...
where tslm_lag >=11;
  ...
  /* watch out, delte this later */ data pred_scrmammo_rcs&i; set model_scrmammo_rcs; keep bene_id month p_scrmammo_rcs ; run;


  /* watch out, delte this later */ proc sort data=pred_scrmammo_rcs&i nodupkey; by bene_id month ;run;
...
data CONTINUE1_&i;
merge bootCONTINUEunique&i (in=a) longcovs&agegroup (keep=newbeneid month scrmammo anymammo tslm_lag tslm );
by newbeneid month; 
if a; 
run;
...
data STOPBASE1_&i;
merge bootSTOPBASEunique&i (in=a) longcovs&agegroup (keep=newbeneid month scrmammo anymammo tslm_lag tslm);
by newbeneid month; 
if a; 
run;
...
%cann23(agein1=75, agein2=76, agein3=77, agein4=78, agein5=79, agein6=80, agein7=81, agein8=82, agein9=83, agein10=84, agegroup=7584 ,from=384 ,to=400 ,event=bcucd_tplusone, debugging= /*obs=1000000*/);
